...
class DirectionServiceImplTest {
	...
	Direction direction;
	@Mock DirectionReactiveRepository directionRepository; 
	...
	@Test
	void testFindById() {
		when(directionRepository.findById(anyString())).thenReturn(Mono.just(direction));
		assertEquals(direction.getDirection(), directionService.findById("1").block().getDirection());
	...
	void testFindDirectionCommandById() {
		when(directionRepository.findById(anyString())).thenReturn(Mono.just(direction));
		assertEquals(d2dc.convert(direction).getDirection(),
				directionService.findDirectionCommandById(anyString()).block().getDirection());
	...
	void testSaveDirectionCommand() {
		...
		when(directionRepository.save(any(Direction.class))).thenReturn(Mono.just(direction));
		DirectionCommand returnedNc = directionService.saveDirectionCommand(nc).block();
	...
	@Test
	void testDelete() {
		when(directionRepository.deleteById(anyString())).thenReturn(Mono.empty());
		directionService.delete("1").block();
